...
/* Global array of registered images and its length -- reallocate as we go */
struct image ** images = NULL;
uint64_t image_count = 0;

#define MAX_THREADS 1000 //macro for number for global arrays

int num_threads[MAX_THREADS] = {0}; //keep track of number of threads 
int curr_op[MAX_THREADS] = {0}; //keep track of thread operating's current turn
sem_t * img_sem; // array of semaphores, one for each image
sem_t * thread_mutex; // array of mutexes, used to keep track of number of threads trying to access an image
sem_t * reg_img_mutex; //mutex for image registration array
sem_t * conn_mutex; //mutex for connection socket
...
int worker_main (void * arg)
{
	...
	while (!params->worker_done || !global_worker_done ) {
		...
		// get a turn number to work on the target image
		sem_wait(&thread_mutex[img_id]);
		int my_turn = num_threads[img_id];
		num_threads[img_id]++;
		sem_post(&thread_mutex[img_id]);

		sem_wait(&img_sem[img_id]);

		while( my_turn != curr_op[img_id]) {
			sem_post(&img_sem[img_id]);
			sem_wait(&img_sem[img_id]);
		}

		// acquire locks on global array of images
		sem_wait(reg_img_mutex);
		...
		assert(img != NULL);
		sem_post(reg_img_mutex); //release mutex
		switch (req.request.img_op) {
			...
			case IMG_HORIZEDGES:
				...
				break;
			case IMG_UNUSED:
				printf("This image operation is not in use\n");
				break;
		}
		sem_wait(reg_img_mutex);
		    ...
		    struct worker_params * common_params)
{
	...
	if (cmd == WORKERS_START) {
		...
		worker_stacks = (char **)malloc(worker_count * sizeof(char *)); // pointer to char pointer (pointer to pointer of stacks)
		...
		for (i = 0; i < worker_count; ++i) {
			...
			worker_stacks[i] = malloc(STACK_SIZE); // pointer to the stack
...
void handle_connection(int conn_socket, struct connection_params conn_params) {
	...
	do {
		...
		if (in_bytes > 0) {
			...
			/* Handle img registration right away */
			if(req->request.img_op == IMG_REGISTER) {
				...
				continue; // skip the entire below
			}
...
int main (int argc, char ** argv) {
	...
	// 1. Images Registration Array
	reg_img_mutex = (sem_t *)malloc(sizeof(sem_t));
	retval = sem_init(reg_img_mutex, 0, 1);
	if (retval < 0) {
		ERROR_INFO();
		perror("Unable to initialize reg_img_mutex ");
		return EXIT_FAILURE;
	}

	// 2. Connection socket 
	conn_mutex = (sem_t *)malloc(sizeof(sem_t));
	retval = sem_init(conn_mutex, 0, 1);
	if (retval < 0) {
		ERROR_INFO();
		perror("Unable to initialize conn_mutex ");
		return EXIT_FAILURE;
	}

	// 3. Semaphore number of threads mutex
	thread_mutex = (sem_t*)malloc(1000 * sizeof(sem_t));

	for (int i = 0; i < 1000; i++) {
		sem_init(&thread_mutex[i], 0, 1);
	}

	// 4. Semaphore locks on each image
	img_sem = (sem_t*)malloc(1000 * sizeof(sem_t));
	for (int i = 0; i < 1000; i++) {
		sem_init(&img_sem[i], 0, 1);
	}
	/* DONE - Initialize image protection variables (semaphores) */
